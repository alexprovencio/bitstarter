...
var checkHtmlFile = function(htmlfile) {
    var checkJson = checkHtml(cheerioHtmlFile(htmlfile), program.checks);
    var outJson = JSON.stringify(checkJson, null, 4);
    console.log(outJson);
};

var checkHtmlURL = function(url) {
    var checkJson = checkHtml(cheerioHtmlURL(url), program.checks);
    var outJson = JSON.stringify(checkJson, null, 4);
    console.log(outJson);
};
...
if(require.main == module) {
    program
        ...
        .option('-url, --url <file_url>', 'Web URL')
	.parse(process.argv);
    if (program.url) {
